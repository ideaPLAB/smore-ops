...
const SECTIONS: WikiSection[] = [
    ...
    body: (
      <>
        <P>
          ...
          기록·검증하는 웹앱이야. 이카운트를 대체하는 게 아니라 <b>앞단의 선검증 원장</b>으로,
          현장에서 벌어지는 입고·출고·판매·조정을 먼저 여기에 기록하고, 검증된 데이터만 이카운트로 보낸다.
        </P>
        <SubTitle>왜 만들었나</SubTitle>
        <P>
          위탁 정산 오류(수량 차이가 정산까지 흘러감), 마이너스 재고 추적 불가, 입력 실수가 ERP에
          그대로 반영되는 문제를 구조적으로 막기 위해서다. 핵심 원칙은 <b>“선검증 후 입력”</b>.
        </P>
        ...
        <P>
          역할 분담: Smore Ops = <b>현장의 진실</b> (실시간 수불), 이카운트 = <b>회계·정산의 공식 기록</b>.
        </P>
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <P>
          모든 재고 변동은 <Code>inventory_events</Code>라는 <b>불변 원장</b>에 이벤트로만 쌓인다.
          재고 수량을 직접 고치는 기능은 없다 — 항상 “무슨 일이 있었는지”를 기록하고, 현재고는 그
          이벤트들의 합으로 계산된다 (<Code>v_stock_balance</Code>). 그래서 어떤 재고든{' '}
          <b>언제·왜 변했는지 끝까지 추적</b>할 수 있다.
        </P>
        ...
        <P>
          창고에서 잔량보다 많은 수량을 출고요청하면 DB가 그 자리에서 차단한다 (트리거).
          즉 마이너스 재고는 <b>발생 자체가 막혀 있다</b>. 관리자 예외 승인과 마이너스 대시보드는
          추후 구현 예정.
        </P>
      </>
    ),
  },
  {
    ...
    body: (
      <>
        ...
        <P>
          확정 순간 시스템이 품목별로 <b>광주 창고재고가 있으면 창고분, 없으면 업체분</b>으로 자동으로 나눈다.
        </P>
        <FlowRow>
        ...
        <P>
          · 판매 데이터가 7일 이상 오래되면 확정이 차단된다 — 먼저 판매 업로드.<br />
          · 확정 취소(전표 회수)를 하면 입력했던 수량은 <b>복원</b>된다. 입력을 비우고 싶으면
          발주판의 <b>RESET</b> 버튼 사용.<br />
          · 제안수량 공식: 주판매 × (배송기간＋1주) ＋ 안전재고 − (매장재고＋이동중), 발주단위 올림.
        </P>
      </>
    ),
  },
    ...
    body: (
      <>
        ...
        <P>
          ...
          <b> 물류</b>는 대기열·입고처리·재고. 물류 계정의 위치는 <b>광주</b>로 등록한다.
        </P>
      </>
    ),
  },
