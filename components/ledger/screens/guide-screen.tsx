...
const GUIDES: Record<Role, GuideSection[]> = {
  manager: [
    ...
    {
      title: '자가사용 처리',
      body: '자가사용은 반드시 포스에 등록합니다 (듀얼아이 포스 등록 필수, 토스 포스는 [할인 > 서비스처리 할인]으로 펀칭). 본사가 매월 1일 전월분을 업로드하면, 매장은 5일까지 자가사용 처리 화면에서 사유·적요를 입력해 차감합니다. 사유가 같은 건은 체크박스로 골라 일괄 저장할 수 있고, 사유 미입력 시 저장되지 않습니다. 이카운트에는 입력하지 않습니다.',
    },
    {
      title: '가챠머신 관리',
